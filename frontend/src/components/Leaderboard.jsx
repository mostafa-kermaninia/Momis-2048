...
function PlayerRow({ player, currentUserData }) {
    ...
    return (
        ...
        >
            <span className="w-8 text-center font-bold text-slate-300 text-lg">{player.rank}</span>
            <div className="flex-1 flex items-center gap-3 ml-4 overflow-hidden">
                ...
                />
                <span className="truncate font-medium text-white">{player.firstName || player.username || "Anonymous"}</span>
            </div>
            <span className="w-16 text-right font-bold text-indigo-300 text-lg">{player.score}</span>
        </li>
    );
}
...
export default function Leaderboard({ API_BASE, onReplay, finalScore, onHome, userData, eventId }) {
    ...
    const banner = finalScore !== null && (
        <div className="mb-4 text-center text-2xl font-bold text-white">
            Your Final Score: <span className="text-yellow-400">{finalScore}</span>
        </div>
    );

    const isCurrentUserInTopList = leaderboard.top.some(p => p.telegramId === leaderboard.currentUser?.telegramId);

    if (loading) {
        return <div className="text-white text-lg animate-pulse">Loading Leaderboard...</div>;
    }
    ...
    if (error) {
        return (
             <div className="w-full max-w-md mx-auto bg-red-800/50 backdrop-blur-xl rounded-2xl shadow-2xl p-6 text-white border border-red-700">
                <h2 className="text-xl font-bold text-center text-red-300 mb-4">Error</h2>
                <p className="text-center">{error}</p>
                 <button onClick={onHome} className="w-full mt-6 py-3 bg-slate-700/80 text-white rounded-lg font-semibold hover:bg-slate-600 transition">
                    Back to Lobby
                </button>
            </div>
        )
        ...
        <div className="w-full max-w-md mx-auto bg-slate-800/50 backdrop-blur-xl rounded-2xl shadow-2xl p-6 text-white border border-slate-700">
            ...
            <ul>
                {leaderboard.top.length > 0 ? (
                    leaderboard.top.map((player) => <PlayerRow key={player.telegramId} player={player} currentUserData={userData} />)
                ) : (
                    <li className="text-center py-8 text-slate-400">No scores yet. Be the first!</li>
                )}
                {!isCurrentUserInTopList && leaderboard.currentUser && (
                    <>
                        <li className="text-center text-slate-500 my-2 font-bold">...</li>
