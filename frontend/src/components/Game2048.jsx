...
const Game2048 = ({ onGameOver }) => {
  ...
  const [bestScore, setBestScore] = useState(() => parseInt(localStorage.getItem('bestScore') || '0', 10));
  ...
  const addRandomTile = (currentTiles, count = 1) => {
    ...
    for (let x = 0; x < size; x++) {
      for (let y = 0; y < size; y++) {
        const isOccupied = currentTiles.some(t => t.x === x && t.y === y);
        if (!isOccupied) {
  ...
  const move = useCallback((direction) => {
    ...
    const vector = { 0: { x: 0, y: -1 }, 1: { x: 1, y: 0 }, 2: { x: 0, y: 1 }, 3: { x: -1, y: 0 } }[direction];
    ...
    traversals.y.forEach(y => {
      traversals.x.forEach(x => {
        const cell = { x, y };
        let tile = currentTiles.find(t => t.x === cell.x && t.y === cell.y);
        ...
        if (tile) {
          ...
          do {
            next = farthest;
            farthest = { x: next.x + vector.x, y: next.y + vector.y };
          } while (farthest.x >= 0 && farthest.x < size && farthest.y >= 0 && farthest.y < size && !currentTiles.some(t => t.x === farthest.x && t.y === farthest.y));
          
          farthest = next; // The actual farthest position

          const nextPos = { x: farthest.x + vector.x, y: farthest.y + vector.y };
          let nextTile = currentTiles.find(t => t.x === nextPos.x && t.y === nextPos.y);
          ...
          if (nextTile && nextTile.value === tile.value && !nextTile.mergedFrom) {
            ...
            currentTiles = currentTiles.filter(t => t.id !== tile.id && t.id !== nextTile.id);
    ...
    const movesAvailable = () => {
        ...
        for (let x = 0; x < size; x++) {
            for (let y = 0; y < size; y++) {
                ...
                if (tile) {
                    for (let d = 0; d < 4; d++) {
                        const vector = { 0: { x: 0, y: -1 }, 1: { x: 1, y: 0 }, 2: { x: 0, y: 1 }, 3: { x: -1, y: 0 } }[d];
                        const otherCell = { x: x + vector.x, y: y + vector.y };
                        const otherTile = tiles.find(t => t.x === otherCell.x && t.y === otherCell.y);
                        if (otherTile && otherTile.value === tile.value) return true;
                    }
                }
            }
        }
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      const map = { 38: 0, 39: 1, 40: 2, 37: 3, 87: 0, 68: 1, 83: 2, 65: 3 };
      ...
      <div className="tile-container">
       ...
       {isGameOver && (
            <div className="game-message game-over">
                <p>Game over!</p>
                <div className="lower">
                    <a className="retry-button" onClick={restartGame}>Try again</a>
                </div>
            </div>
        )}
        {won && !isGameOver && (
