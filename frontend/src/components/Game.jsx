    ...
    const move = useCallback(
        (dx, dy) => {
            ...
            for (const tile of sortedTiles) {
                ...
                // این حلقه تا زمانی که کاشی می‌تواند حرکت کند، ادامه می‌یابد
                while (true) {
                    ...
                    if (
                        ...
                        break; // از محدوده خارج شد
                    }

                    const blockingTile = sortedTiles.find(
                        (t) => t.row === nextRow && t.col === nextCol
                    );

                    if (blockingTile) {
                        if (
                            blockingTile.value === tile.value &&
                            !blockingTile.isMerged
                        ) {
                            blockingTile.value *= 2;
                            blockingTile.isMerged = true;
                            newScore += blockingTile.value;

                            const index = sortedTiles.findIndex(
                                (t) => t.id === tile.id
                            );
                            sortedTiles.splice(index, 1);
                            hasChanged = true;
                        }
                        break; // به یک کاشی دیگر برخورد کرد
                    }

                    // اگر مانعی نبود، یک خانه حرکت کن
                    row = nextRow;
                    col = nextCol;
                }
        ...
        const checkGameOver = () => {
            ...
            for (const tile of tiles) {
                ...
                const rightNeighbor = tiles.find(
                    (t) => t.row === row && t.col === col + 1
                );
                if (rightNeighbor && rightNeighbor.value === value)
                    return false;

                const bottomNeighbor = tiles.find(
                    (t) => t.row === row + 1 && t.col === col
                );
                if (bottomNeighbor && bottomNeighbor.value === value)
                    return false;
            }
            return true;
        };
        ...
        <div {...swipeHandlers} className="game-wrapper" tabIndex={-1}>
            <div className="game-container">
                ...
                <div className="game-intro">
                    <span>
                        Join the numbers, get to the <strong>2048 tile!</strong>
                    </span>
