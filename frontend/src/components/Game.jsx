...
KeyboardInputManager.prototype.emit = function(event, data) { const callbacks = this.events[event]; if (callbacks) { callbacks.forEach(c => c(data)); } };
KeyboardInputManager.prototype.listen = function() {
  const map = { 38: 0, 39: 1, 40: 2, 37: 3, 75: 0, 76: 1, 74: 2, 72: 3, 87: 0, 68: 1, 83: 2, 65: 3 };
  document.addEventListener("keydown", event => {
    const modifiers = event.altKey || event.ctrlKey || event.metaKey || event.shiftKey;
    const mapped = map[event.which];
    if (!modifiers && mapped !== undefined) { event.preventDefault(); this.emit("move", mapped); }
  });
};

function LocalStorageManager() {
  this.bestScoreKey = "bestScore"; this.gameStateKey = "gameState"; this.storage = window.localStorage;
}
...
LocalStorageManager.prototype.getGameState = function() { return JSON.parse(this.storage.getItem(this.gameStateKey)); };
...
const Game = ({ onGameOver, onExit }) => {
  useEffect(() => {
    ...
    const originalActuate = gameManager.actuator.actuate.bind(gameManager.actuator);
    ...
    const restartButtons = document.querySelectorAll('.retry-button, .restart-button');
